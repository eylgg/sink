...
Dungeon entrances are drawn with the map's own blue portal (the `Dungeon` atlas) and their level range after the name, such as "Ruins of Lordaeron (11-24)". The tooltip lists the dungeon's quests: a red cross for one not in your log, a yellow waiting mark for one in it, a green check for one you have done, in that order and alphabetical within each. A dungeon entry has `dungeon = true`, `levels`, `atlas` and `quests`, a list of `{ id, name }` whose name stands in until the client has the quest cached. "Show dungeons" turns them off.
