...
Quests linked by `after` make a series. The step follows the name on the map tooltips, "Hidden Enemies (3/5)" on Ragefire Chasm and "(1/5)" on Thrall, though not on the first quest of a series that starts inside a dungeon, where "(Kill "The Baron" inside)" says enough, and the objective tracker adds it to the end of the title: "Unending Torment (2/5)". Sink hooks the tracker's `UpdateSingle`, which sets each quest's header on every update, and appends the step to the header text; if the longer title would wrap onto another line, Blizzard's title is kept so the tracker layout never breaks.
