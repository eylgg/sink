...
From these the map draws each dungeon's entrance with its quests and a "Dungeon Quest" pin on each NPC who gives one. A drop whose NPC is inside the quest's dungeon reads "Kill "The Baron" inside", an NPC inside it who gives the quest reads "From "Nalpak" inside", and an item on the ground inside it reads "Loot inside": "Crest of Lordaeron (Loot inside)". Both are yellow until done, never red.
